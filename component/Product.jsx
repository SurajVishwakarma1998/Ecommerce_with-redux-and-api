...
import {addToCart,increse,decrese} from "../redux/Cart";

import {fetchProducts} from "../redux/ProductReducer";
import {STATUSES} from "../redux/ProductReducer";
  ...
  return (
    <Container >
          <Row>
            ...
            {products.map((data)=> 
              <Col key={data.id} >              
                  <Card  className=' m-3 d-flex flex-row p-1'>
                  <Container className='d-flex flex-column ' style={{width:"30%"}} >
                        <Container className='d-flex flex-row mt-3' style={{justifyContent:"center"}}>
                          <Card.Img  src={data.image} style={{width:"140px",height:"220px",margin:"10px auto"}} />
                         </Container>
                         <Container className='d-flex flex-row mt-3' style={{justifyContent:" space-around"}}>
                          {/* <AiOutlineMinusCircle color='#DC4C64' size={28}  />
                               <span style={{width:"30px",display:"flex",justifyContent:"center",alignItems:"center",backgroundColor:"#B2B2B2"}}>{quantity}</span> 
                           <AiOutlinePlusCircle color='#3B71CA' size={28}  /> */}
                          </Container>
                      </Container>
                     <Container className='d-flex flex-column'>
                      <Card.Body>
                        <Card.Title>{(data.title).slice(1,40)}  <Badge bg="success">{data.rating.count} In Stock</Badge></Card.Title>
                            <Card.Text>{(data.description).slice(1,100)} </Card.Text>
                      </Card.Body>
                    <ListGroup className="list-group-flush">
                      <ListGroup.Item style={{fontWeight:"500"}} ><BiCategory size={20} color="#009EFF"/> {data.category}</ListGroup.Item>
                      <ListGroup.Item><GiPriceTag size={20} color="#FF5B00" /> ${data.price} per unit  </ListGroup.Item>
                      <ListGroup.Item><FcRating size={20} /> rating : {data.rating.rate}/5 </ListGroup.Item>
                    </ListGroup>
                    <Card.Body onClick={() => redirect('/cart')}>
                       <Button onClick={()=>dispatch(addToCart({id:data.id,quantity,title:data.title,image:data.image,price:data.price,count:data.rating.count,description:data.description,category:data.category,rating:data.rating.rate}))}><FaCartPlus size={20} /> Add to cart</Button>
                       <Button className='bg-info mx-1' ><AiFillDollarCircle size={20} color="#EB6440"/> Pay Now</Button>
                    </Card.Body>
                    </Container>
                  </Card>      
                </Col>                                
